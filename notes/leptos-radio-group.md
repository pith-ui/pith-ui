...
2. **`composeRefs`/`useComposedRefs`**: Used via `pith-ui-compose-refs::use_composed_refs` where multiple refs are needed (RadioGroupItem has local ref + forwarded ref, RadioIndicator has forwarded ref + presence ref).
