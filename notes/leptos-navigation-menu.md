...
5. **`core-primitive` dependency** — The React source imports from `@radix-ui/primitive` for `composeEventHandlers`. In Leptos, `compose_callbacks` from `pith-ui-primitive` serves the same purpose, so no separate `core-primitive` dependency is needed.
...
4. **Indicator positioning**: Uses `Portal` component from `pith-ui-portal` with `mount` prop targeting the indicator track `RwSignal<Option<SendWrapper<HtmlElement>>>`. The indicator impl uses `use_resize_observer` on both the active trigger and track to compute position.
