...
pub use pith_ui_roving_focus::Orientation;
...
- **Orientation re-export:** `Orientation` is re-exported from `pith_ui_roving_focus` rather than defining a separate type. This avoids type conversion issues when passing orientation to `RovingFocusGroup`. Default horizontal is set in the component via `unwrap_or(Orientation::Horizontal)`.
...
- **Event composition:** Uses `compose_callbacks` from `pith-ui-primitive` to merge user-provided event handlers with internal handlers.
