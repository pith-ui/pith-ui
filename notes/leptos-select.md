...
4. **Collection pattern**: Uses the shared `pith-ui-collection` crate with `CollectionProvider`, `CollectionSlot`, and `CollectionItemSlot` for item tracking, matching the pattern established by other ported components.
