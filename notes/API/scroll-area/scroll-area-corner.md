...
- The corner reports its measured dimensions back to the parent `ScrollArea` context, which uses them to set `--radix-scroll-area-corner-width` and `--radix-scroll-area-corner-height` CSS custom properties on the root element. The scrollbars use these properties to offset themselves so they don't overlap the corner.
